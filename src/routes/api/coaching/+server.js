...
${last4Weeks.filter(a => a.user_notes).length > 0 ? `## COMMENTAIRES DE L'ATHLÈTE (notes personnelles sur ses séances)
${last4Weeks.filter(a => a.user_notes).map(a => `${a.activity_date?.split('T')[0]} — ${a.name}: "${a.user_notes}"`).join('\n')}
Prends en compte ces commentaires pour adapter le plan (douleurs, fatigue, contraintes, motivations).` : ''}
...
Vérifie que les endurances fondamentales sont bien faites en Z1-Z2 (<75% FC max). Si l'athlète passe trop de temps en Z3+ sur les sorties faciles, signale-le.` : ''}
